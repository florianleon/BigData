...
def find_best_index(liste, cmp, inert=False):
    distances = []
    best_index = 0
    for i in range(len(liste)):
        if (inert == True):
            distance = compute_distance([0.0, 0.0], [i, liste[i]])
        else:
            distance = compute_distance([0.0, 1.0], [i, liste[i]])
...
def get_best_k():
    ...
    best_inert = find_best_index(inerts, 0.0, True)
    best_homs = find_best_index(homs, 1.0)
    # Calcul de la moyenne des metrics
    mean_k = (best_silh + best_v_meas + best_homs) / 3
    ...
    print("Best inert", best_inert)
    print("Best silh", best_silh)
    print("Best v_meas", best_v_meas)
    print("Best homs", best_homs)
    print("------------------------------------------------------")
...
hom = metrics.homogeneity_score(ground_truth, model_km.labels_)
v_mea = metrics.v_measure_score(ground_truth, model_km.labels_)
print("Inertie : ", inert)
print("Coefficient de silhouette : ", silh)
print("Coefficient d'homogénéité : ", hom)
print("Coefficient de v-mesure : ", v_mea)
"""
